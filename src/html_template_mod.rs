...
pub trait HtmlServerTemplateRender {
    ...
    fn render_sub_template(
        ...
        _prefixes: &PrefixForTemplateVariables,
    ) -> Vec<Node> {
...
pub fn render<T: HtmlServerTemplateRender>(
    self_0: &T,
    html_template_raw: &str,
    server_or_client_prefixes: ServerOrClient,
) -> String {
    ...
    let nodes = unwrap!(render_template_raw_to_nodes(
        self_0,
        &html_template_raw,
        HtmlOrSvg::Html,
        "",
...
/// returns Nodes
pub fn render_template_raw_to_nodes<T: HtmlServerTemplateRender>(
    self_0: &T,
    html_template_raw: &str,
    ...
    let sub_templates = extract_children_sub_templates(html_template_raw, prefixes);
...
fn fill_element_node<T: HtmlServerTemplateRender>(
    self_0: &T,
                    ...
                    Token::Attribute(name, value) => {
                        if retain_this_node == true {
                            if name.starts_with(&prefixes.attr_text) {
                                ...
                                let repl_bool = self_0.retain_next_node_or_attribute(placeholder);
                    ...
                    Token::Comment(txt) => {
                        if retain_this_node == true {
                            ...
                            if txt.starts_with(&prefixes.text) {
                                ...
                                retain_next_node_or_attribute =
                                    self_0.retain_next_node_or_attribute(txt);
                                ...
                                let repl_vec_nodes = self_0.render_sub_template(
                                    template_name,
                                    sub_templates,
                                    prefixes,
                                );
...
fn extract_children_sub_templates(
    template_raw: &str,
    prefixes: &PrefixForTemplateVariables,
) -> Vec<SubTemplate> {
...
// endregion: ? private functions for trait HtmlServerTemplateRender
